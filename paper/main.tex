...
\author{Nischal Dhakal and [Supervisor Name]%
\thanks{N. Dhakal and [Supervisor] are with the School of Automation, Chongqing University of Posts and Telecommunications, Chongqing 400065, China. E-mail: \texttt{[email]}}%
...
\begin{figure}[t]
\centering
\textcolor{red}{[PLACEHOLDER: System architecture diagram --- RISE-MAPPO $\to$ Lyapunov-MPC $\to$ Distributed GP]}
...
Let $\mathbf{h}_i \in \Rbb^d$ be the feature vector for agent~$i$ extracted from the shared backbone, and let $\sigma_i$ be the GP posterior standard deviation at robot~$i$'s position. The attention weight for agent~$i$ is:
\begin{equation}
w_i = \frac{\exp\!\big(\mathbf{q}^\top \mathbf{h}_i \cdot (1 + \eta \, \tilde{\sigma}_i)\big)}{\sum_{j=1}^{N} \exp\!\big(\mathbf{q}^\top \mathbf{h}_j \cdot (1 + \eta \, \tilde{\sigma}_j)\big)},
\label{eq:attention}
\end{equation}
where $\mathbf{q} \in \Rbb^d$ is a learnable query vector, $\tilde{\sigma}_i = \sigma_i / \max_j \sigma_j$ is the normalized uncertainty, and $\eta \geq 0$ is a temperature hyperparameter controlling the strength of the uncertainty modulation. When $\eta = 0$, the mechanism reduces to standard dot-product attention.

The attended global feature is:
\begin{equation}
\mathbf{g} = \textstyle\sum_{i=1}^{N} w_i \, \mathbf{h}_i,
\label{eq:attended}
\end{equation}
which is then fed to both the $\vmean$ and $\vcvar$ heads. This design requires no additional communication beyond what CTDE already provides (all agent features are available to the centralized critic during training) and adds negligible computational cost since the attention is a single softmax over $N$ agents.
...
We evaluate RISE-MAPPO in a custom multi-robot search environment implemented in Python with Gymnasium. The robots follow the TurtleBot3 Burger kinematic model~\eqref{eq:dynamics} with parameters listed in Table~\ref{tab:params}. The MPC is formulated in CasADi and solved with IPOPT. Local GPs use GPyTorch with 50 inducing points and a squared-exponential kernel.
...
\textcolor{red}{[Coverage rate, detection success rate, collision rate, energy efficiency, mean CVaR risk, exploration overlap, Lyapunov stability (monotonic fraction). 50 episodes per scenario, 5 seeds. Statistical testing: Wilcoxon signed-rank with Cliff's delta effect size.]}

\subsection{Main Results}
\textcolor{red}{[Table~III: main comparison across all baselines on Complex scenario. Coverage curves (Fig.~X). Discussion of risk--return trade-off.]}
...
\textbf{Limitations.} The current evaluation uses a 2D simulated environment with idealized sensing. The GP kernel hyperparameters are fixed rather than adapted online, which may limit performance in highly heterogeneous environments. The Lyapunov contraction constraint can become infeasible near obstacle boundaries, requiring a fallback to softer constraints.
